...
public:
	...
	//virtual yarp::os::Bottle& findGroup(const char*){};
	 ...
	 */
	static eValue * makeBlob(const char* p, const unsigned int size);
...
public:
	 ...
	 *
	 * \param[in] x The vocab to insert
	 */
	void addVocab(int x);
	...
	/**
	 * Removes a Value v from the end of the list
	 *
	 ...
	 */
	void fromBinary(const char * p, int size);
	 ...
	 */
	const char * const toBinary(int *size = 0) const;
	...
	/**
	 * This callback is called when the writting operations start
	 */
	void onCommencement();

	/**
	 * Check if there exists a property of the given name.
	 * \param[in] key The name of the property to check
	 ...
	 * \param[in] key The keyword of the value to find
	 * \return The value corresponding to the key
	 */
	virtual eValue & find(const char *key);

	/**
	 * Gets a list corresponding to a given keyword.
	 */
	eBottle & findGroup(const char *key);
	 ...
	 * Rebuilds the eBottle as a copy of another one
	 *
	 * \param[in]  b A reference to the eBottle to copy from.
	 * \param[in] first The index of the first element to copy
	 * \param[in] len The number of elements to copy (-1 for all)
	 */
	void copy(const eBottle& b, int first = 0, int len = -1);
	 ...
	 * Check if there exists a property of the given name
	 */
	virtual bool check(const char *key, const char *comment);

	/**
	 * Gets a value corresponding to a given keyword
	 */
	virtual bool check(const char *key, eValue *&result, const char *comment =
			0);

	/**
	 * Gets a value corresponding to a given keyword
	 */
	virtual eValue check(const char *key, const eValue &fallback,
			const char *comment = 0);

	/**
	 * Gets a list corresponding to a given keyword
	 */
	eBottle & findGroup(const char *key, const char *comment);
	...
	/////////////////////////////////////////////////////////////////////////
	// eBottle addons not in yarp::os::Bottle
	/////////////////////////////////////////////////////////////////////////
